...
NEEDED = ["VIXCLS", "BAMLH0A0HYM2", "BAMLC0A0CM", "NFCI", "SOFR", "DFF"]
...
def _to_wide_df(session) -> pd.DataFrame:
    ...
    return df.pivot_table(index="date", columns="series_id", values="value", aggfunc="last").sort_index()
...
def _build_scores(wide: pd.DataFrame) -> pd.DataFrame:
    ...
    # TEDRATE is discontinued; remove it entirely from the score system.
    panic = 100.0 * (0.30 * p_vix + 0.25 * p_hy + 0.15 * p_ig + 0.15 * p_nfci + 0.15 * p_repo)
    liq = 100.0 * (0.45 * p_repo + 0.25 * p_nfci + 0.30 * p_hy)

    out = pd.DataFrame({"panic_score": panic, "liquidity_score": liq}).dropna()
    if out.empty:
        return out
